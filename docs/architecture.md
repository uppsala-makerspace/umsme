...
| Send membership reminder mails | Daily at 09:00 (configurable) | `admin/server/cronjob/sendReminders.js` | Mirrors the manual "Send reminder" button: looks up a non-deprecated reminder template, sends mail, inserts a Messages row, and pushes. Per-template Messages-row dedup prevents double sends. Gated: runs only when `reminderCron.enabled === true` **and** `deliverMails` is truthy, so a dev/staging instance with a copied DB never sends. |
